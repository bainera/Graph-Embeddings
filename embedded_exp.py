...
if __name__ == '__main__':
    ...
    for i in range(0,10):
        ...
        for j in range(0,10):     # how accurate is the exp
                ...
                print "iteration number: "+str(j)
                print "\n\n\n"
                g_tag,Matrix2 = create_Random_graph(0.5,n)
        ...
        for i in range(0,n):
            for j in range(0,n):
                if Matrix2[i][j]==1:
                    total_edges +=1
        if total_edges!=0:
            num_of_removed_edges.append((sum_removed/10)/total_edges)
        else:
            num_of_removed_edges.append((sum_removed/10))
    ...
    plt.ylabel('Average of Number of edges have been removed from Original Graph (in 10 experiments) / num of total edges ')
