...
def create_graph():
    ...
    for vertex in range (0,n):
        ...
        num_of_neighbors =randint(0,4)     ## Here we can play with the degree of each node!!
        s = set()
...
def show_graphs(g,t_spanner,Matrix2):
...
def Tests(g,t_spanner,n,t,edes_between_clusters,clusters):
    ...
    if ans:
        print "\n\nComparing Test Passed!"
    else:
        print "\n\nComparing Test Failed!"
    print " \n\n#################################\n\n"
...
def start():
    ...
    t =  int(lines[0][2:])
    n = int(lines[1][2:])
    g_tag,Matrix2 = create_graph()

    covered = {}
    clusters = []
    # [[all vertices in the cluster so far] [uncovered vertices that are neighbors of the cluster] [number of vertices in the cluster so far], ... ,[ [] [] [] ],    ]
    #
    ...
    for cc in c:
        ...
        for k in cc[0]:
            print k
    print "H a T z L a H a\n"

    #no_dups_list = remove_duplicates_from_Matrix(edges_between_every_two_clusters)
    for i in no_dups_list:
        print str(i[0].getId())+"," + str(i[1].getId())
    print "##### ORIGINAL GRAPH ####\n"

    for key,value in g_tag.vertList.iteritems():
       print value

    print "##### T-SPANNER GRAPH ####\n"
    ...
    show_graphs(g_tag,t_spanner,Matrix2)
